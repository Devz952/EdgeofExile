...
The optional `d2rl.multiplayer` block gives multiplayer a safe name for exact mismatch messages.

Use `shared` when the patch changes gameplay that every player must share. Use `host` only when the host alone decides the result. Use `client` only for local presentation such as rendering or UI changes.

```json
"d2rl": {
  "multiplayer": {
    "id": "author.patch-name",
    "role": "shared"
  }
}
```

The ID uses 1 through 64 lowercase ASCII letters, numbers, `.`, `_`, or `-`. Keep it stable across releases. Two enabled Shared patches in the same scope must not use the same ID.

If this block is missing, the patch safely defaults to `shared`. It still participates in compatibility, but a mismatch can only say that memory patches differ. Invalid metadata also defaults to `shared` and produces a warning.

Shared patches are compared by their final effective memory writes. Formatting, descriptions, expected safety bytes, and splitting the same write across operations do not change the fingerprint. Changed addresses or resulting bytes do change it. Host and client patches are excluded from multiplayer admission.

Public game listings may contain only the patch scope, stable ID, and a comparison digest. Patch filenames, paths, addresses, RVAs, and raw bytes are never published.
